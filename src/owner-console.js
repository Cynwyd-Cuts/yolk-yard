...
export class OwnerConsole {
  ...
  async request(path,options={}){
    ...
    const response=await fetch(url+path,{...options,headers:{...(options.body?{'Content-Type':'application/json'}:{}),...(this.token?{'Authorization':`Bearer ${this.token}`}:{})},cache:'no-store'});
  ...
  logout(){void this.request('/logout',{method:'POST'}).catch(()=>{});this.token=null;this.expires=0;this.dialog.close();}
}
