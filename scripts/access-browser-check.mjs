...
try {
 ...
 await guest.getByRole('button',{name:'Pause menu'}).click();
 await guest.getByRole('button',{name:'Settings',exact:true}).click();
 await guest.getByRole('button',{name:'Bind Move forward primary',exact:true}).click();await guest.keyboard.press('KeyI');
 await guest.getByRole('button',{name:'Bind Fire primary',exact:true}).click();await guest.keyboard.press('KeyF');
 await guest.getByRole('button',{name:'Bind Aim / scope primary',exact:true}).click();await guest.keyboard.press('KeyC');
 assert.equal(await guest.evaluate(()=>JSON.parse(localStorage.getItem('yolk-settings')).keybinds.forward[0]),'KeyI');
 await guest.getByRole('button',{name:'Done',exact:true}).click();await guest.getByRole('button',{name:'RESUME',exact:true}).click();
 const remapPos={x:p.x,z:p.z};await guest.keyboard.down('KeyI');await wait(()=>Math.hypot(p.x-remapPos.x,p.z-remapPos.z)>.3);await guest.keyboard.up('KeyI');
 const remapAmmo=p.ammo[0];await guest.keyboard.down('KeyF');await wait(()=>p.ammo[0]<remapAmmo);await guest.keyboard.up('KeyF');
 await guest.keyboard.down('KeyC');await wait(()=>p.aim);await guest.keyboard.up('KeyC');await wait(()=>!p.aim);
 await guest.getByRole('button',{name:'Pause menu'}).click();await guest.getByRole('button',{name:'Settings',exact:true}).click();
 await guest.getByRole('button',{name:'Reset default keybinds',exact:true}).click();
 assert.equal(await guest.evaluate(()=>JSON.parse(localStorage.getItem('yolk-settings')).keybinds.forward[0]),'KeyW');
 await guest.getByRole('button',{name:'Done',exact:true}).click();await guest.getByRole('button',{name:'RESUME',exact:true}).click();
 console.log('PASS remapped movement, firing, scope, persisted bindings and reset');
