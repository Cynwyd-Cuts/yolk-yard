...
try{
 ...
 await host.keyboard.press('Escape');await host.locator('[data-action="leave-confirm"]').click();await host.getByRole('button',{name:'Close room',exact:true}).click();await guest.getByText('Connection ended',{exact:true}).waitFor();pass('Host departure ends the room clearly');
