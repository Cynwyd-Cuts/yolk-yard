...
test('round rules bound invalid values and use mode-specific defaults',()=>{
 ...
 assert.equal(matchOptions({mode:'royale',minutes:2}).minutes,0);
 assert.equal(matchOptions({minutes:Infinity}).minutes,5);
 const o=matchOptions({minutes:900,scoreLimit:-2,bots:3.7,difficulty:99});
 assert.deepEqual([o.minutes,o.scoreLimit,o.bots,o.difficulty],[60,1,4,4]);
});
test('configured score targets end both arena modes, not the old hardcoded target',()=>{
...
test('time limit and rematch reconfiguration preserve people and replace old bots',()=>{
 ...
 const round=s.round;
 assert.equal(s.configure({map:MAPS[1].id,mode:'teams',minutes:12,scoreLimit:5,bots:1,difficulty:3}),true);
