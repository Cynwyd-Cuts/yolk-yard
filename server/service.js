...
export function createService({origin,adminSecret,store,rooms,serve,secure=true,trustProxy=false,limits=new Map()}) {
  ...
  const handler=async(req,res)=>{
    ...
    try {
      ...
      if (['/access.js','/access.css','/admin.js'].includes(path)) return await serve(res,path);
  ...
  return {handler, requirePlayer, limit};
}
